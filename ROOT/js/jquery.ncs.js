...
var goLnb = function(obj, s_seq, sbj_no) {
	...
	} else if ($(obj).val() == "/ncsMgt/operation/impReEvaluation.do") {
		goDetailM($(obj).val(), {'params' : {'s_seq' : s_seq, 'stdt_no' : null, 'se_ord' : null}});
	} else {
...
var showNCSUnitInfo = function(cd) {
	var cd1 = cd.substr(0, 2);
	var cd2 = cd.substr(2, 2);
	var cd3 = cd.substr(4, 2);
	var cd4 = cd.substr(6, 2);
	var cd5 = cd.substr(8, 2);

	window.open("http://www.ncs.go.kr/ncs/page.do?sk=P1A2_PG01_002&mk=MU00000067&uk=MU00000067&ncsLclasCd=" + cd1 + "&doCompeUnit=true&ncsMclasCd=" + cd2 + "&ncsSclasCd=" + cd3 + "&ncsSubdCd=" + cd4 + "&ncsClCd=" + cd + "&doSubd=true&ncsCompeUnitCd=" + cd5 + "&ncsSubdCdnm=");
}
	...
	$(this).ajaxSubmit({
		...
		, success : function (result, textStatus) {
			...
			if( result.result > 0 ){
				...
				if(result.returnMsg != null && options.returnMsg != ''){
